...
* Linux players install the agent instead of building it: ghostty-agent ships as a Fedora RPM and as a portable tarball for every other glibc machine, both built from source inside a Fedora container and attached to every release, with a systemd user unit so your shells start with your session and outlive the game. The Fedora 44 package carries the Wayland compositor, so screens from your desktop and the browser work; the portable build has terminals, jobs and clips. Asked for by @Xe. Built and installed in a container, not yet on a player's own Fedora machine.
