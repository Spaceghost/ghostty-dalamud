...
impl UdpSocketState {
    pub fn new(socket: UdpSockRef<'_>) -> io::Result<Self> {
        ...
        let v6only = unsafe {
            ...
            if rc == -1 {
                // Vendored change (ghostty-dalamud): IPV6_V6ONLY is queried on
                // every socket, including AF_INET ones. Real Windows answers;
                // Wine returns WSAEOPNOTSUPP (10045) and this bind died, which
                // is what kept the plugin core from opening an iroh endpoint at
                // all. An IPv4 socket is definitionally not v6-only, so the
                // failure is recoverable: take false and carry on.
...
/// Vendored change (ghostty-dalamud): the same option, but a refusal from the
/// network stack is not fatal.
///
/// The game runs under Wine, whose winsock does not implement every option
/// Windows does -- IP_RECVECN in particular is recent. quinn treats all of
/// these as required, so one missing option stopped the endpoint from binding
/// at all ("bind IPv4 failed: OS Error 10042"). Each of them only adds
/// information: without IP_PKTINFO a datagram's destination address is
/// unknown, without IP_RECVECN there is no ECN, without DONTFRAGMENT a packet
/// may fragment. QUIC works in every one of those cases, more slowly at worst.
/// On real Windows these calls succeed and nothing here changes.
fn set_socket_option_soft(
    ...
    match set_socket_option(socket, level, name, value) {
        ...
        Err(e) => match e.raw_os_error() {
            // WSAENOPROTOOPT, WSAEOPNOTSUPP, WSAEINVAL: the stack does not
            // know this option. Anything else is a real failure.
            Some(10042) | Some(10045) | Some(10022) => Ok(()),
            _ => Err(e),
        },
    }
}
