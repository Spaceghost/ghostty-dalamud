...
screen when it is adopted is moved inside it (`igSetWindowPos_WindowPtr`) and
moved back on release. Release also gives back the plugin's own
`NoMouseInputs` bit. Nothing else about the window is changed.
...
| File | Role |
|---|---|
| `core/adopt.nelua` | pure: window ↔ panel mapping, CPU clipping and strips, the draw-list copy, snapshots, pointer remap and the mouse event rewrite, keeping on screen, flags, the adopt/lose/give-up state machine, the grip's geometry, window names |
