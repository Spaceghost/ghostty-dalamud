...
| `test_agent_caps` | the capability words of the agent's greeting (core/agent_caps.nelua): the round trip, every state, older greetings staying unknown, unknown keys and values skipped, over-long or split words refused |
| `test_agentcaps.lua` | what the Settings tab says about the agent (lua/agentcaps.lua): the title, one hint per missing feature, Windows needing no compositor, an older agent told to update, no agent |
