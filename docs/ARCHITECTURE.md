...
agent/        embedded WireGuard (docs/WIREGUARD.md): crypto.nelua (Monocypher), blake2s.nelua, wg_proto (constants, replay window, addresses), wg_device (handshake, transport, cookies, timers), wg_udp (the socket), wg_netstack (lwIP inside the tunnel and the loopback forwards), wg_config (wireguard.conf), wg_service (all of it in the agent's loop)
