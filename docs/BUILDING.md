...
The gaming PC (`alienware`) has 15 GB of RAM and runs the game and a local
LLM. A full `tools/build.sh` there gets killed for low memory, so **the
default way to build this project is in an Incus container on a build host**,
...
| Host | Does | Does not |
| --- | --- | --- |
| `alienware` (this machine) | installs into the game, runs the agent, `/term selftest` in the running game, `tools/install-dev.sh`, git | compile |
| `fedora` | every heavy build and the whole test suite, in the `ghostty-build` container. 8 cores, 62 GB | touch the game |
| `bak` | Atlas hardware flashing and HIL only | build — no build ever goes to `bak` |

`fedora` is an Incus remote on this machine (`incus remote list`), so the
scripts talk to its daemon over TLS; nothing here needs an ssh session or a
shell on the host. If a machine has no `fedora` remote:

```sh
incus remote add fedora https://fedora.<domain>:8443        # preferred
INCUS='ssh fedora incus' tools/build-container.sh create    # fallback, if incus over ssh is all there is
```
...
incus image copy fedora:ghostty-build local: --alias ghostty-build
...
```sh
incus copy fedora:ghostty-build otherhost:ghostty-build
incus move fedora:ghostty-build otherhost:ghostty-build
```
...
On 2026-09-19, against `fedora` (8 cores, 62 GB), container `ghostty-build`
...
and the image exists on `fedora`, but there is no second build host to copy it
to (`bak` does not build). The in-game parts are unchanged and still local.
