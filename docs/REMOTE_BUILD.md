...
## Local or remote is decided for you

`tools/build.sh` and `tests/run.sh` place themselves: on the build host unless
the game is not running here and memory is free, and moved there if the game
starts mid-build. The rule, the `FORCE_BUILD_HOST` override, what eviction does
and how to see where a build ran are in [BUILD_PLACEMENT.md](BUILD_PLACEMENT.md).
The commands above always use the build host. The same placement serves the
other projects through `build-place` (`tools/run-placed.sh`), one script, not a
copy per repository.
...
2026-09-19, container `ghostty-build` on the build host (Fedora 44, 8 CPUs, 32 GiB), tree
...
directory `remote-build-2026-09-19/` beside the checkout, together with a
