...
local A = {
  ...
  -- that refused to start it). A.not_found_text() words it for the platform;
  -- setting this field replaces the whole text.
...
-- What to do when the assistant command is missing, said for the platform the
-- game runs on. Where the command has to exist differs:
--
--   native Windows: on this Windows machine when the agent is the local
--     ghostty-agent.exe (or the ConPTY/pipe fallback runs it in the game
--     process), on the remote machine when CONFIG.agent points at one.
--   Wine/Proton: on the Linux or macOS host, where ghostty-agent runs.
--
-- almanac is a Python project, so a Windows player who does not want it there
-- has two other ways: run ghostty-agent on a machine that does have it (the
-- agent transport carries the conversation over the network, and the panel's
-- JSON arrives through a pipe, so nothing reflows it), or use whatever
-- assistant they do have by pointing CONFIG.assistant.stream / .threads at it.
local NOT_FOUND = {
  windows = 'almanac was not found on this Windows machine. Three ways on: install almanac here, '
    .. 'so that `almanac` is on PATH for the user who runs the game; or run ghostty-agent on a machine '
    .. 'that has almanac and point CONFIG.agent (host, port, token_file) at it, with '
    .. "CONFIG.assistant.transport = 'agent'; or point CONFIG.assistant.stream and .threads "
    .. '(lua/assistant.lua) at another program that prints the same JSON lines. '
    .. '`/ask term` opens the assistant in a terminal instead of the panel.',
  wine = 'almanac was not found where ghostty-agent runs (the Linux or macOS host, not the game\'s '
    .. 'Wine prefix). Install almanac there so that `almanac` is on PATH for the user who runs the '
    .. 'agent, or point CONFIG.assistant.stream / .threads (lua/assistant.lua) at another program. '
    .. '`/ask term` opens the assistant in a terminal instead of the panel.',
}

-- `cfg.not_found` when it was set (a text of one's own wins), else the wording
-- for `platform_name` (ghostty.platform(); the plain text for anything else).
function A.not_found_text(cfg, platform_name)
  cfg = cfg or A
  if cfg ~= A and type(cfg.not_found) == 'string' and cfg.not_found ~= '' and cfg.not_found ~= A.not_found then
    return cfg.not_found
  end
  if not platform_name then
    local ok, platform = pcall(require, 'platform')
    if ok then platform_name = platform.name() end
  end
  return NOT_FOUND[platform_name] or tostring(cfg.not_found or A.not_found)
end
...
function A.exit_message(status, refused, cfg, platform_name)
  cfg = cfg or A
  local lines = { '' }
  if refused or status == 127 then
    lines[#lines + 1] = '\27[33m' .. A.not_found_text(cfg, platform_name) .. '\27[0m'
  end
  local what
  if refused then
