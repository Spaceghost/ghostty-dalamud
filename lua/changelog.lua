...
C.releases = {
  {
    ...
    items = {
      ...
      { 'next', 'Pets bump instead of clipping: they keep off each other, other world screens, you and your target, and stop short of the game\'s walls, pillars, floors and ceilings, sliding round to where there is room, using the game\'s own collision. A pet never floats through anything: where its way is blocked it blinks out and pops back in on the other side. /term world rays shows what the game\'s collision tells them. Characters you have not targeted are not avoided yet. Host tests only; not yet tried in game.' },
      { 'next', 'Cuter pets: springy movement, a gentle bob on each pet\'s own beat, a small tilt of their own, stacked pets fanned out and peeking over each other instead of standing in columns, a bank as they swing after you, a squash when they bump or land, and a little procession as they follow you. Settings, Pets: Reduce motion turns it all off. Host tests only; not yet tried in game.' },
