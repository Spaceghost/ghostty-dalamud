...
C.releases = {
  {
    ...
    items = {
      ...
      { 'beta', 'A camera that hovers around you: /term cam puts one on an orbit beside your character, and /term cam shot makes the view its view, so it frames you instead of looking out of your eyes. Move it with left, right, up, down, in and out, set it circling with orbit, and turn the shot off to have the view eased back exactly where it was found. The game draws one view, so the camera borrows it rather than adding a second one; taking a picture from it without disturbing your own view is the next piece. Presenting a screen takes the camera back. Not yet tried in game.' },
      ...
      { 'next', 'Pets bump instead of clipping: they keep off each other, other world screens and you, and stop short of the game\'s walls, pillars, floors and ceilings, sliding round to where there is room, using the game\'s own collision. A pet never floats through anything: in a tight gap it squeezes small and slips through, it floats up over low things, it shuffles round a pillar, and only where there is no way (a wall, a door you walked through) does it blink out and pop back in on the other side. Mobs and NPCs are kept clear of as they come (a short one walks under a pet as it tucks up its hem and floats up a little); other players walking past are let pass, and only one who stays gets room. Lamp posts and other props the game keeps on other collision layers now count too. /term world rays shows what the game\'s collision tells them. Host tests only; not yet tried in game.' },
