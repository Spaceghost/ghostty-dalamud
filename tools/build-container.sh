...
#
# Environment:
#   INCUS_REMOTE   Incus remote to work on. Default: `fedora` when
#                  `incus remote list` has it, else `local`.
...
# The remote: an explicit INCUS_REMOTE, else the `fedora` remote when this
# machine has one, else the local daemon.
if [[ -z "${INCUS_REMOTE:-}" ]]; then
  if "$INCUS" remote list --format csv 2>/dev/null | cut -d, -f1 | grep -qx fedora; then
    INCUS_REMOTE=fedora
  else
    INCUS_REMOTE=local
    warn "no 'fedora' Incus remote on this machine; using the local daemon.
  Add one with:  incus remote add fedora https://fedora.<domain>:8443
  Without a remote the fallback is to run incus over ssh: INCUS='ssh fedora incus'"
  fi
fi
...
cmd_update() {
  ...
  if [[ "$zv" != "$ZIG_VERSION" ]]; then
    log "fedora's zig is '${zv:-none}', toolchain.env pins $ZIG_VERSION: fetching the pinned release"
    insh "set -e
