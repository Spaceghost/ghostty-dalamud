...
# This is the default way to build: the gaming PC has 15 GB of RAM and runs the
# game, so a full build there gets killed for low memory.
#
...
# Also:
#   INCUS_REMOTE       which Incus host (default: the `fedora` remote)
...
remote_run() { # remote_run <label> <script>
  ...
  run_in "set -e
    ...
    flock $BUILD_CONTAINER_CACHE/build.lock -c '$2'
    echo '--- sccache'
